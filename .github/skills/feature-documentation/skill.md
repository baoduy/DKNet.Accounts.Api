...
```
Domain Modeling   → DKNet.Accounts.Domains/Features/ManualSample/Entities/PurchaseOrder.cs
EF Mapping        → DKNet.Accounts.Infra/Features/ManualSample/Mappers/PurchaseOrderConfigs.cs
CRUD Handlers     → DKNet.Accounts.AppServices/ManualSample/V1/Actions/
Domain Events     → DKNet.Accounts.AppServices/ManualSample/V1/Events/
API Endpoints     → DKNet.Accounts.Api/ApiEndpoints/ManualSample/PurchaseOrderV1Endpoint.cs
```
...
|-------|-------------------------------|
| `DKNet.Accounts.Api` | Route mapping only; no business logic |
| `DKNet.Accounts.AppServices` | Command handling, validation, event publishing |
| `DKNet.Accounts.Domains` | Entity state, domain rules, invariants |
| `DKNet.Accounts.Infra` | Persistence, EF Core config, message bus setup |
````
...
> `MapGetList` route documents a different contract: `pageNumber`/`pageSize` defaulting 1/1000 with a
> configurable ceiling of 1000 (`ListQueryOptions`, section `DKNet:ListQuery`), plus `fromDate`/`toDate`
> last-activity bounds that window a bare listing over audited records to the last three months. Use
...
See `DKNet.Accounts.Infra/Features/ManualSample/Mappers/PurchaseOrderConfigs.cs` for the full config.
...
**Published by**: `PurchaseOrder` constructor (via `AddEvent`), delivered by `DKNet.Accounts.Infra/Services/EventPublisher.cs` after `SaveChangesAsync`
...
See `DKNet.Accounts.Infra/Extensions/ServiceBusSetup.cs` for the bus wiring.
