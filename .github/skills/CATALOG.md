...
- **Prerequisites**: Existing feature implementation and working `DKNet.Accounts.App.BDDTests` infrastructure
- **Success Criteria**:
   ...
   - `dotnet test ApiEndpoints/DKNet.Accounts.App.BDDTests` passes
...
1. **Start with Domain Modeling**: 
   - Create your entity in `DKNet.Accounts.Domains` with your mapper in `DKNet.Accounts.Infra`
   - See [Domain Modeling skill.md](./domain-modeling/skill.md)

2. **Add Business Logic with CRUD**:
   - Create request DTOs with validators in `DKNet.Accounts.AppServices`
   - Create repository and domain events
   - See [CRUD Operations skill.md](./crud-operations/skill.md)

3. **Expose via API**:
   - Create endpoints in `DKNet.Accounts.Api` with OpenAPI docs
...
→ Ensure mapper class is `sealed` and in `DKNet.Accounts.Infra/Features/<Feature>/Mappers/`  
