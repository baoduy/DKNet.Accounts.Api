...
- **Endpoint location**: `DKNet.Accounts.Api/ApiEndpoints/<Feature>/<Entity>V1Endpoint.cs`
- **Command/Query handlers**: `DKNet.Accounts.AppServices/<Feature>/V1/Actions/` or `/Queries/`
- **Validators**: same file as command, internal sealed class
- **Services**: `DKNet.Accounts.Infra/Services/` (`internal sealed`, `.Services` namespace)
- **Domain aggregate**: `DKNet.Accounts.Domains/Features/<Feature>/Entities/` (UpperCamelCase)
- **Event handlers**: `DKNet.Accounts.AppServices/<Feature>/V1/Events/`
- **Repositories**: Infra layer — use `IRepositorySpec` with a `Specification`; do not add a per-entity repo interface
- **EF Core config**: `DKNet.Accounts.Infra/Features/<Feature>/Mappers/<Entity>Configs.cs`
- **Migrations**: `DKNet.Accounts.Infra/Migrations/` — run `dotnet ef migrations add <Name> -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj` from `ApiEndpoints/`
...
- **Pagination**: if applicable, use `DKNet.AspCore.Extensions`' `MapGetList` (pageNumber/pageSize, defaults 1/1000, ceiling 1000, configurable via `ListQueryOptions` in section `DKNet:ListQuery`, plus `fromDate`/`toDate` last-activity bounds defaulting to the last 3 months over audited records).
...
- [ ] Features organized under `DKNet.Accounts.AppServices/<Feature>/V1/`.
...
- [ ] EF configurations under `DKNet.Accounts.Infra/Features/<Feature>/Mappers/`.
- [ ] Migrations created with `dotnet ef migrations add ... -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj`.
