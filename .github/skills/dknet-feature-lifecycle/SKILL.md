...
| # | Path | `manual` | `auto` |
|---|---|---|---|
| 1 | `DKNet.Accounts.Domains/Features/<Feature>/Entities/` | entity + hand-written event record(s) | entity only — `[RaisesEvent]`/`[CrudCreate]`/`[CrudUpdate]`/`[CrudAction]` carry the rest |
| 2 | `DKNet.Accounts.Infra/Features/<Feature>/Mappers/` | `IEntityTypeConfiguration<T>` | same — **no generator produces this** |
| 3 | `DKNet.Accounts.Infra/Features/<Feature>/StaticData/` | optional `IDataSeedingConfiguration<T>` | optional |
| 4 | `DKNet.Accounts.Infra/Features/<Feature>/ExternalEvents/` | optional broker consumer | optional broker consumer |
| 5 | `DKNet.Accounts.AppServices/<Feature>/V1/` | `Actions/`, `Queries/`, `Specs/`, `Events/`, `<Feature>Dto.cs` | `<Feature>Dto.cs` (one `[GenerateDto]` line) + `Events/` consumers only |
| 6 | `DKNet.Accounts.Api/ApiEndpoints/<Feature>/` | `<Entity>V1Endpoint.cs` — every route a literal `Map*` call | `<Entity>V1Endpoint.cs` — one `group.Map<Entity>Crud()` call |
| 7 | `DKNet.Accounts.App.Tests/Unit/<Feature>/` | entity/validator/spec tests | entity/handler tests |
| 8 | `DKNet.Accounts.App.Tests/Integration/<Feature>/V1/` | result-level handler + security tests | same |
| 9 | `DKNet.Accounts.App.BDDTests/Features/<Plural>/` | `*.feature` + `Steps/*.cs` | same |
...
|---|---|---|
| Schema constant | `DKNet.Accounts.Domains/Share/DomainSchemas.cs` | if the feature added its own `const string` |
| Broker topology | `DKNet.Accounts.Infra/Extensions/ServiceBusSetup.cs` | the `azb.Produce<T>` / `azb.Consume<T>` pair, e.g. `ProductCreatedEvent` on `product-tp` |
| Feature flag | `DKNet.Accounts.Share/Options/FeatureOptions.cs` + `appsettings*.json` `FeatureManagement` section | if the feature gated itself behind a flag |
| EF migration | `DKNet.Accounts.Infra/Migrations/` | see §4 — never hand-delete an applied migration |
...
```bash
ls ApiEndpoints/DKNet.Accounts.Domains/Features/
```
...
- **Not applied and it is the newest migration** — `cd ApiEndpoints && dotnet ef migrations remove -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj`
- **Applied, or newer migrations sit on top of it** — do NOT touch the old migration. Delete the
  entity and mapper, then `dotnet ef migrations add Drop<Feature> -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj` and let EF emit the drop. Rewriting
