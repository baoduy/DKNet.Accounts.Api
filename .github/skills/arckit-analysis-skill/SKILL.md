...
- **API layer**: Find dknet.accounts API endpoints or endpoint configs (Api/ApiEndpoints/).
- **Command/Query layer**: Locate handlers under `DKNet.Accounts.AppServices/<Feature>/V1/Actions/` and `/Queries/`.
...
```
grep -r "IHandler<YourEvent>" DKNet.Accounts.AppServices/ → Found consumers under <Feature>/V1/Events/
```
