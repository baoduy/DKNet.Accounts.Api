...
mapper discovery from `DKNet.Accounts.Infra/Features/<Feature>/Mappers`. No fluent API scattered across the
...
- **Mapping**: Mapster (global configuration in `DKNet.Accounts.AppServices/AppSetup.cs`)
- **Validation**: FluentValidation
- **Messaging**: In-memory bus + Azure Service Bus (optional)
- **Hosting**: Optional Aspire orchestration (`DKNet.Accounts.AppHost`)
...
Must align with `DKNet.Accounts.Share/Options/FeatureOptions.cs`. Never hardcode feature state; always
...
- **Tests**: Live under `DKNet.Accounts.App.Tests/<Category>/<Feature>.cs` (Unit, Integration, Architecture).
