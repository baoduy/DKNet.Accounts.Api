...
2. `ApiEndpoints/DKNet.Accounts.AppServices/ManualSample/V1/` (exemplar: `Actions/{Create,Update,Cancel,Delete}.cs`, `Specs/SpecGetPurchaseOrder.cs`, `Queries/{GetPurchaseOrderById,ListPurchaseOrders}.cs`, `Events/`, `PurchaseOrderDto.cs`)
...
For a genuinely plain CRUD entity, declare the CRUD surface on the entity instead of writing it. Exemplar: `Product` (`ApiEndpoints/DKNet.Accounts.Domains/Features/AutomatedSample/Entities/Product.cs`, `ApiEndpoints/DKNet.Accounts.AppServices/AutomatedSample/V1/ProductDto.cs`).
...
`DKNet.SlimBus.Generators` produces (namespace `DKNet.Accounts.AppServices.Crud`, not committed — inspect via `dotnet build` then `obj/Generated/DKNet.SlimBus.Generators/`):
...
- Acting-user attribution cannot use `[FromClaim]` on a generated request (the generator forwards only `System.ComponentModel.DataAnnotations` attributes) — it goes through `DKNet.EfCore.AuditLogs`' audit hook instead — `ICurrentUserProvider.GetCurrentUser()`, wired once in `DKNet.Accounts.Api/Configs/ServiceConfigs.cs`, not per-entity.
