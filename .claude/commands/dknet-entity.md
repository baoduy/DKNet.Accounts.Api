---
description: Scaffold a new domain aggregate (entity + owned types + EF Core mapper + migration) inside a DKNet.DKNet.Accounts solution.
...
3. The exemplar for the selected mode:
   - `manual` — `ApiEndpoints/DKNet.Accounts.Domains/Features/ManualSample/Entities/PurchaseOrder.cs` (hand-written entity, raises its own event via `AddEvent(...)`; no declarative attribute)
   - `auto` — `ApiEndpoints/DKNet.Accounts.Domains/Features/AutomatedSample/Entities/Product.cs` (class-level `[RaisesEvent]`, `[CrudCreate]` ctor, `[CrudUpdate]` and `[CrudAction]` methods; no `AddEvent` anywhere) plus `docs/crud-attributes.md`
4. `ApiEndpoints/DKNet.Accounts.Infra/Features/ManualSample/Mappers/PurchaseOrderConfigs.cs` (exemplar mapper — hand-written in **both** modes; no generator produces this)

## Steps

1. Use the `dknet-implementer` subagent (via `Task`) to execute Steps 1–4 of the implementer protocol: domain entity, schema constant, owned types, EF Core mapper, optional sequence/seed data, then `dotnet ef migrations add <Name> -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj` from `ApiEndpoints/`.
...
3. **`auto` only** — confirm the composed event-record names against the compiled assembly; they have
   ...
   ```bash
   strings ApiEndpoints/DKNet.Accounts.Domains/bin/Release/net10.0/DKNet.Accounts.Domains.dll | grep <Entity>
   ```
...
- Do NOT touch `DKNet.Accounts.AppServices` or `DKNet.Accounts.Api` here — those belong to `/dknet-crud` and `/dknet-endpoint`.
