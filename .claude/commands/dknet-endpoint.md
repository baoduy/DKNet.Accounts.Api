---
description: Add a DKNet.Accounts API IEndpointConfig that exposes CRUD actions for a DKNet feature with idempotency on POST.
...
2. `ApiEndpoints/DKNet.Accounts.Api/ApiEndpoints/ManualSample/PurchaseOrderV1Endpoint.cs` (exemplar — every route is a literal `group.MapPost/MapGet/MapPut/MapDelete(...)` call against the raw dknet.accounts-API surface, base route `/v1/purchase-orders`)
...
- Do NOT add controllers or attribute routing — this is DKNet.Accounts API only.
