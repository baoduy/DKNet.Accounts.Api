...
`DKNet.Accounts.Domains/Features/`), plus optional `--dry-run`.
...
1. Resolve the feature: confirm `ApiEndpoints/DKNet.Accounts.Domains/Features/<Feature>/` exists. If it
...
2. BDD — `ApiEndpoints/DKNet.Accounts.App.BDDTests/Features/<Plural>/` (both `*.feature` and the
   generated `*.feature.cs`, plus `Steps/`).
3. Tests — `DKNet.Accounts.App.Tests/Unit/<Feature>/` and `DKNet.Accounts.App.Tests/Integration/<Feature>/`.
   Also grep `DKNet.Accounts.App.Tests/Architecture/` — a convention test may assert on this feature by name.
4. Api — `DKNet.Accounts.Api/ApiEndpoints/<Feature>/`.
5. AppServices — `DKNet.Accounts.AppServices/<Feature>/`.
6. Infra — `DKNet.Accounts.Infra/Features/<Feature>/`.
7. Domains — `DKNet.Accounts.Domains/Features/<Feature>/`.
...
1. `DKNet.Accounts.Domains/Share/DomainSchemas.cs` — drop the feature's `const string`, if it added one.
   Leave `Migration` and `Profile` alone unless this feature owned one of them.
2. `DKNet.Accounts.Infra/Extensions/ServiceBusSetup.cs` — drop the matching `azb.Produce<T>` /
   `azb.Consume<T>` pair and the now-unused `using`.
3. `DKNet.Accounts.Share/Options/FeatureOptions.cs` + every `appsettings*.json` `FeatureManagement` section
...
- Feature's migration is the newest and unapplied → `cd ApiEndpoints && dotnet ef migrations remove -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj`
- Otherwise → `cd ApiEndpoints && dotnet ef migrations add Drop<Feature> -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj` and verify the generated `Up`
