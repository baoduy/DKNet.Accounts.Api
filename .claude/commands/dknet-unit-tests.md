...
You are adding integration tests in `DKNet.Accounts.App.Tests` that exercise the AppServices and Domains layers through the real DI container.
...
2. `ApiEndpoints/DKNet.Accounts.App.Tests/` — existing fixtures and test patterns (`Architecture/`, `Integration/`, `Unit/`).
...
2. Run only the affected tests:
   ```
   dotnet test ApiEndpoints/DKNet.Accounts.App.Tests/DKNet.Accounts.App.Tests.csproj --filter "FullyQualifiedName~<Entity>"
   ```
