...
5. The exemplar slice for any layer where you're unsure — this template ships two, and `docs/samples/manual-vs-automated.md` is the authoritative layer-by-layer comparison between them:
   - **Hand-written (primary walkthrough below)** — `DKNet.Accounts.Domains/Features/ManualSample/Entities/PurchaseOrder.cs`, `DKNet.Accounts.Infra/Features/ManualSample/Mappers/`, `DKNet.Accounts.AppServices/ManualSample/V1/Actions/`, `Specs/`, `Queries/`, `Events/`, `DKNet.Accounts.Api/ApiEndpoints/ManualSample/PurchaseOrderV1Endpoint.cs`.
   - **Generator-driven (faster path, plain CRUD only — see below)** — `DKNet.Accounts.Domains/Features/AutomatedSample/Entities/Product.cs`, `DKNet.Accounts.AppServices/AutomatedSample/V1/ProductDto.cs`, `DKNet.Accounts.Api/ApiEndpoints/AutomatedSample/ProductV1Endpoint.cs`.
...
4. **EF migration** — `cd ApiEndpoints && dotnet ef migrations add <Name> -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj`. Inspect the generated migration before continuing.
...
6. **Api endpoint** — new `*V1Endpoint : IEndpointConfig`; map every route with literal `group.MapPost/MapGet/MapPut/MapDelete(...)` calls against the raw dknet.accounts-API surface (see `PurchaseOrderV1Endpoint`). Add `.RequiredIdempotentKey()` to the POST chain — clients then send `X-Idempotency-Key: {Guid}`; a replayed key returns the original response instead of creating a duplicate.
...
- `[CrudCreate]` on the constructor and `[CrudUpdate]` on a mutation method — `DKNet.SlimBus.Generators` then generates the request record, handler, and route registration for you (namespace `DKNet.Accounts.AppServices.Crud`, not committed — inspect `obj/Generated/DKNet.SlimBus.Generators/` after a build).
