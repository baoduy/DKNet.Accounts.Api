...
description: Orientation to the DKNet.DKNet.Accounts.Template layer boundaries, vertical-slice folder layout, and auto-discovery wiring. Use first, before any other dknet-* skill, when working in a solution generated from this template.
---
...
- First skill to read when starting any feature work in a DKNet.DKNet.Accounts.Template solution (or a solution generated from it).
...
```
DKNet.Accounts.Api          → entry point, endpoints, auth, OpenAPI
  ↓
DKNet.Accounts.AppServices  → CQRS handlers, validators, DTOs, domain event handlers
  ↓
DKNet.Accounts.Domains      → entities, aggregate roots, repo interfaces
  ↑
DKNet.Accounts.Infra        → EF Core (CoreDbContext), repos, event publisher, service bus
  (wires into Api via InfraSetup.AddInfraServices)

DKNet.Accounts.Share        → shared constants/options/base types (read by all layers)
DKNet.Accounts.AppHost      → Aspire orchestration only (Redis + PostgreSQL + DKNet.Accounts.Api), no business logic
```
...
- `DKNet.Accounts.Domains/Features/ManualSample/Entities/PurchaseOrder.cs`
- `DKNet.Accounts.Infra/Features/ManualSample/Mappers/`, `StaticData/`
- `DKNet.Accounts.AppServices/ManualSample/V1/Actions/`, `Specs/`, `Events/`
- `DKNet.Accounts.Api/ApiEndpoints/ManualSample/PurchaseOrderV1Endpoint.cs`

**`Product`** (generator-driven CRUD + events):
- `DKNet.Accounts.Domains/Features/AutomatedSample/Entities/Product.cs` (`[RaisesEvent]`, `[CrudCreate]`, `[CrudUpdate]`, `[CrudAction]`)
- `DKNet.Accounts.Infra/Features/AutomatedSample/Mappers/` (still hand-written), `ExternalEvents/`
- `DKNet.Accounts.AppServices/AutomatedSample/V1/ProductDto.cs` (`[GenerateDto]`), `Events/`
- `DKNet.Accounts.Api/ApiEndpoints/AutomatedSample/ProductV1Endpoint.cs` (`group.MapProductCrud()`)
