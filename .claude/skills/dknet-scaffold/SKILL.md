---
name: dknet-scaffold
description: Scaffold a new solution from the DKNet.DKNet.Accounts template and get it running — dotnet new install/new, the six template parameters, what the generated tree looks like, first run with or without Aspire, and deleting the two shipped sample features. Use when starting a new DKNet.DKNet.Accounts solution, or when orienting inside a freshly generated one.
---

# Scaffolding a DKNet.DKNet.Accounts solution
...
```bash
dotnet new install DKNet.DKNet.Accounts.Template
dotnet new dknet-dknet.accounts -n <YourApp>
cd <YourApp>
```
...
| `--RepositoryUrl` | `https://github.com/baoduy/DKNet.Accounts.Api` | Project metadata `RepositoryUrl`. |
...
```
<YourApp>/
├── DKNet.Accounts.sln                  ← the solution; `dotnet build` from here needs no path
...
└── ApiEndpoints/
    ├── DKNet.Accounts.Api/             ← endpoints, auth, OpenAPI
    ├── DKNet.Accounts.AppServices/     ← CQRS handlers, validators, DTOs
    ├── DKNet.Accounts.Domains/         ← entities, aggregate roots
    ├── DKNet.Accounts.Infra/           ← EF Core, repos, event publisher
    ├── DKNet.Accounts.Share/           ← shared constants/options
    ├── DKNet.Accounts.AppHost/         ← Aspire orchestration
    ├── DKNet.Accounts.App.Tests/       ← xUnit + Shouldly
    ├── DKNet.Accounts.App.BDDTests/    ← Reqnroll + NUnit
    └── DKNet.Accounts.App.TestSupport/
```
...
`ApiEndpoints/DKNet.Accounts.Domains/...`. (In the DKNet.Templates repo itself the same tree is nested under
`src/`.)
...
```bash
cd ApiEndpoints
dotnet ef migrations add <Name>   -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj
dotnet ef migrations remove       -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj
```
