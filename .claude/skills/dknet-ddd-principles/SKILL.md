...
- Aggregates reference each other by ID (`Guid`), never by object reference. `PurchaseOrder` (`DKNet.Accounts.Domains/Features/ManualSample/Entities/PurchaseOrder.cs`) does not hold a `Customer` object — it holds a plain `CustomerName` string, no navigation property back to another aggregate.
...
Both styles are delivered identically afterward: `DKNet.Accounts.Infra/Services/EventPublisher.cs` forwards to `IMessageBus` regardless of which raised the event.
