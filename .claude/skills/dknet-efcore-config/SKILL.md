...
```
ApiEndpoints/DKNet.Accounts.Infra/
├── Features/
│   └── {Feature}/
│       ├── Mappers/
...
Create `ApiEndpoints/DKNet.Accounts.Infra/Features/{Feature}/Mappers/{Entity}Configs.cs`. Both samples
...
namespace DKNet.Accounts.Infra.Features.{Feature}.Mappers;
...
`dknet-ddd-principles`), register it in `ApiEndpoints/DKNet.Accounts.Infra/Contexts/OwnedDataContext.cs`:
...
Create `ApiEndpoints/DKNet.Accounts.Infra/Features/{Feature}/StaticData/{Entity}StaticData.cs`,
...
namespace DKNet.Accounts.Infra.Features.{Feature}.StaticData;
...
Create `ApiEndpoints/DKNet.Accounts.Infra/Services/{Service}.cs`:
...
**Critical**: Class MUST be `sealed` and in the `DKNet.Accounts.Infra.Services` namespace for Scrutor auto-registration.

### Step 5: Run Migration

```bash
cd ApiEndpoints
dotnet ef migrations add {MigrationName} -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj
```

Verify the generated migration in `ApiEndpoints/DKNet.Accounts.Infra/Migrations/`.
...
```csharp
// DKNet.Accounts.Infra/Features/ManualSample/Mappers/PurchaseOrderConfigs.cs
...
// DKNet.Accounts.Infra/Features/AutomatedSample/Mappers/ProductConfigs.cs
...
```csharp
// DKNet.Accounts.Infra/Services/MembershipService.cs
...
- [ ] File placed in `DKNet.Accounts.Infra/Features/{Feature}/Mappers/`
- [ ] Service implementations are `internal sealed` in `.Services` namespace
- [ ] Migration generates cleanly: `dotnet ef migrations add {Name} -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj`
