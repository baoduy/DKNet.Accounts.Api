...
description: Create DDD domain entities following this project's AggregateRoot/DomainEntity inheritance pattern. Use when adding a new domain entity or owned type to DKNet.Accounts.Domains.
---
...
> `DKNet.Accounts.Domains/Features/ManualSample/Entities/PurchaseOrder.cs`. An entity can instead declare
...
> `DKNet.Accounts.Domains/Features/AutomatedSample/Entities/Product.cs`. The trade-off: the generated request's
...
AuditedEntity<Guid>        ← from DKNet.EfCore.Abstractions.Entities
  └── DomainEntity          ← DKNet.Accounts.Domains.Share (abstract, Guid Id + audit)
       └── AggregateRoot    ← DKNet.Accounts.Domains.Share (abstract, Guid auto-gen)
```
...
```
ApiEndpoints/DKNet.Accounts.Domains/
├── Features/
│   └── {Feature}/
│       └── Entities/
...
Create `ApiEndpoints/DKNet.Accounts.Domains/Features/{Feature}/Entities/{Entity}.cs`. Mirror
...
namespace DKNet.Accounts.Domains.Features.{Feature}.Entities;
...
This is a direct mirror of `DKNet.Accounts.Domains/Features/ManualSample/Entities/PurchaseOrder.cs` — read
...
Edit `ApiEndpoints/DKNet.Accounts.Domains/Share/Sequences.cs` to add sequence name:
...
`Product` (`DKNet.Accounts.Domains/Features/AutomatedSample/Entities/Product.cs`) reaches the same outcome
...
- [ ] Namespace follows `DKNet.Accounts.Domains.Features.{Feature}.Entities`
- [ ] File placed in `ApiEndpoints/DKNet.Accounts.Domains/Features/{Feature}/Entities/`
