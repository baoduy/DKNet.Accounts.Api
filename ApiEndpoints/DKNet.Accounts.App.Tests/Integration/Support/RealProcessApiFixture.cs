...
/// <c>DKNet.Accounts.Api.dll</c> — already copied into this test project's own output directory as a
...
public abstract class RealProcessApiFixtureBase : IAsyncLifetime
{
    ...
    /// <summary>
    ///     Everything the child <c>DKNet.Accounts.Api</c> process wrote to stdout/stderr. Redirected streams MUST be
    ...
    {
        Port = GetFreeTcpPort();
        var dllPath = Path.Combine(AppContext.BaseDirectory, "DKNet.Accounts.Api.dll");
        ...
        _process = Process.Start(startInfo) ?? throw new InvalidOperationException("failed to start DKNet.Accounts.Api process.");
    ...
    private async Task WaitUntilReadyAsync()
    {
        ...
        throw new TimeoutException(
            $"DKNet.Accounts.Api process did not become ready within 30s.{Environment.NewLine}{ProcessOutput}");
    }
