...
///     (via <c>TokenHandlers.Clear()</c>) must never reappear in <c>DKNet.Accounts.Api</c>.
/// </summary>
public class JwtSignatureValidationTests
{
    ...
    [Fact]
    public void DKNet_AccountsApiSource_ShouldNotDisableJwtSignatureValidation()
    {
        var apiSourceDir = Path.GetFullPath(
            Path.Combine(AppContext.BaseDirectory, "../../../../../ApiEndpoints/DKNet.Accounts.Api"));
        ...
        offenders.ShouldBeEmpty(
            "DKNet.Accounts.Api must rely on stock JWT bearer signature validation — none of its source files " +
