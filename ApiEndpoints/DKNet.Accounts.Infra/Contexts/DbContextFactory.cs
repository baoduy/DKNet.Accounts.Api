...
internal sealed class DbContextFactory : IDesignTimeDbContextFactory<CoreDbContext>
{
    ...
    /// domain events through <see cref="DKNet.Accounts.Infra.Services.EventPublisher"/>, which needs an
    ...
    /// runtime path passes — <c>DKNet.Accounts.Infra</c> cannot reference <c>DKNet.Accounts.Api</c>, so the app
    /// assembly is unreachable at design time. Today that's harmless: no aggregate is seeded yet. A future
    /// <c>DataSeedingConfiguration&lt;T&gt;</c> over an event-raising entity would need a declared producer
    /// for its event to be handled here.
    /// </summary>
