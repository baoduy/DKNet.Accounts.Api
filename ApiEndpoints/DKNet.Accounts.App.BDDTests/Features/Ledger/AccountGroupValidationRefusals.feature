Feature: Account group refusals decided by validation
  ...
  Scenario: A group is created when its code is free
    Given no account group holds the code "TROPS"
    When treasury-ops creates a group with code "TROPS" and name "Treasury Operations"
    Then the group is created
    And the response carries the code "TROPS"

  @new @integration
  Scenario: A group is refused when its code is taken
    Given an account group holds the code "TROPS"
    When treasury-ops creates another group with code "TROPS"
  ...
  Scenario: Two simultaneous creates of one code leave one group
    Given no account group holds the code "TROPS"
    When two requests create a group with code "TROPS" at the same moment
    Then one request creates the group
    And the other is refused and the refusal carries the code "DUPLICATE_GROUP_CODE"
    And exactly one account group holds the code "TROPS"

  @new @integration
  Scenario Outline: A group is not closed while an account still holds money
    Given the account group "TROPS" holds an account with a balance of <balance> SGD and a held amount of <held> SGD
    When treasury-ops closes the group "TROPS"
    Then the request is refused with 422
  ...
  Scenario: A group is closed when every account it holds is at zero
    Given the account group "TROPS" holds one account with a balance of 0.00 SGD and a held amount of 0.00 SGD
    When treasury-ops closes the group "TROPS"
    Then the group is closed
    And the response carries the group "TROPS"

  @new @integration
  Scenario: A group holding no account is closed
    Given the account group "TROPS" holds no account
    When treasury-ops closes the group "TROPS"
    Then the group is closed

  @new @integration
  Scenario: A closed group is reopened
    Given the account group "TROPS" is closed
    When treasury-ops reopens the group "TROPS"
    Then the group is active
    And the response carries the group "TROPS"

  @new @integration
  Scenario Outline: A caller holding only the read permission cannot change a group
    Given the account group "TROPS" is active
    And auditor-ops holds the account-group read permission and not the write permission
    When auditor-ops <action> the group "TROPS"
    Then the request is refused with 403
    And the group "TROPS" is still active
  ...
  Scenario: The status-only partial update is no longer offered
    Given the account group "TROPS" is active
    When treasury-ops sends a partial update of the group "TROPS" carrying only the status "Closed"
