...
internal sealed class AccountsV1Endpoint : IEndpointConfig
{
    ...
    public void Map(RouteGroupBuilder group)
    {
        ...
        // GetList now registers through one generated composite (DRK-1522 §3 row 12), same as account groups.
        // Accounts publish no delete route (R1: never reachable today) — the generated composite's Delete
        // stays excluded by name. GetById/Rename/ChangeMetadata are excluded too and hand-mapped below with an
        // explicit "{id:guid}" pattern (pr-reviewer round 1, finding 7): the generated composite's own default
        // is the looser "{id}", which answers a malformed id 400 instead of the 404 this service has always
        // answered and the README still documents — the same reason PostingsV1Endpoint.MapGetById below keeps
        // its own explicit "{id:guid}" against that same default.
        group.MapAccountCrud(o => o
                .Exclude("Delete", "GetById", "Rename", "ChangeMetadata")
