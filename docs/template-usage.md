...
dotnet new install DKNet.DKNet.Accounts.Template --nuget-source "https://nuget.pkg.github.com/baoduy/index.json"
```

## Scaffold a solution

```bash
dotnet new dknet-dknet.accounts -n MyCompany.MyService
```
...
|---|---|---|---|---|---|
| Name | `-n`, `--name` | string | `MyDKNet.AccountsApp` | — (`sourceName`: `DKNet.Accounts`) | Not a `replaces` symbol. `sourceName` rewrites the identifier `DKNet.Accounts` in every file name, folder name, namespace and project reference, and renames `DKNet.Templates.sln` to `<Name>.sln`. |
...
| RepositoryUrl | `--RepositoryUrl` | string | `https://github.com/baoduy/DKNet.Accounts.Api` | `https://github.com/baoduy/DKNet.Accounts.Api` | `<PackageProjectUrl>` and `<RepositoryUrl>` in `Directory.Packages.props`. |
...
```bash
dotnet new dknet-dknet.accounts -n Acme.OrderService \
...
└── MyCompany.MyService.ApiEndpoints/
    ├── MyCompany.MyService.Api/           # DKNet.Accounts API entry point
...
`src/DKNet.DKNet.Accounts.Template.nuspec` is what decides. These are the same AI-assistant
...
```bash
cd src
dotnet pack DKNet.DKNet.Accounts.Template.csproj -c Release -o ./nupkgs

# Test locally before publishing
dotnet new install ./nupkgs/DKNet.DKNet.Accounts.Template.1.0.0.nupkg
dotnet new uninstall DKNet.DKNet.Accounts.Template

# Publish
dotnet nuget push ./nupkgs/DKNet.DKNet.Accounts.Template.1.0.0.nupkg \
