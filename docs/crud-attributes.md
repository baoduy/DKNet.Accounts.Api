...
`DKNet.Accounts.Domains/Features/AutomatedSample/Entities/Product.cs`:
...
- **`[GenerateDto(typeof(Product))]`** — `DKNet.Accounts.AppServices/AutomatedSample/V1/ProductDto.cs`:
...
`DKNet.Accounts.AppServices/Extensions/MapsToExtensions.cs`'s `ScanMaps` reflects over the assembly for
either attribute and calls `config.NewConfig(entityType, dtoType)` for each hit. That scan runs from
`DKNet.Accounts.AppServices/AppSetup.cs` at startup; no per-feature mapping registration is needed for
either style.
...
- Their handlers, in the `DKNet.Accounts.AppServices.Crud` namespace: `CreateProductHandler`,
...
opt-in is one start-up registration in `DKNet.Accounts.Api`, described in
...
`ApproveProductHandler` — in the `DKNet.Accounts.AppServices.Crud` namespace. They are compiler output
...
|---|---|---|
| HTTP endpoint | `DKNet.Accounts.Api/ApiEndpoints/ManualSample/PurchaseOrderV1Endpoint.cs` — literal `group.MapPost("/", ...)` etc., one call per route | `DKNet.Accounts.Api/ApiEndpoints/AutomatedSample/ProductV1Endpoint.cs` — one `group.MapProductCrud()` call, nothing hand-mapped |
| Validation | `CreatePurchaseOrderCommandValidator`/`UpdatePurchaseOrderCommandValidator` (FluentValidation) run and are enforced on every hand-mapped route | `[Range]`/`[Required]` on the `[CrudCreate]`/`[CrudUpdate]` parameters are forwarded onto the generated request but **not evaluated** — `MapProductCrud` routes through `DKNet.AspCore.Extensions`'s generic `Map*<TRequest,TDto>` wrapper, which the .NET 10 dknet.accounts-API validation source generator can't see through. A negative price returns `201`, not `400` |
...
| Handler | `CreatePurchaseOrderCommandHandler`/`UpdatePurchaseOrderCommandHandler` (`DKNet.Accounts.AppServices/ManualSample/V1/Actions/`) | Generated `CreateProductHandler`/`ChangePriceProductHandler` (`DKNet.Accounts.AppServices.Crud` namespace) |
...
The hand-written path's explicit actions — `DKNet.Accounts.AppServices/ManualSample/V1/Actions/Create.cs`,
...
has one — `DKNet.Accounts.Infra/Features/ManualSample/StaticData/PurchaseOrderStaticData.cs`:
...
- `DKNet.Accounts.Infra/Extensions/InfraSetup.cs` → `AddInfraServices` (the DI-registered context the
  running app uses)
- `DKNet.Accounts.Infra/Extensions/InfraMigration.cs` → `MigrateDb` (the startup-migration path)
...
`DKNet.Accounts.App.Tests/Integration/ManualSample/V1/InfraMigrationSeedingTests.cs`, calls
...
`DKNet.Accounts.Domains/Share/Sequences.cs`:
...
`ISequenceServices`/its base `SequenceService` (`DKNet.Accounts.Infra/Services/SequenceService.cs`) wrap
