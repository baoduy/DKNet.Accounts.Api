...
`IMediator`. Wiring lives in `DKNet.Accounts.Infra/Extensions/ServiceBusSetup.cs`:
...
`AutoDeclareFrom` scans `serviceAssembly` — the `DKNet.Accounts.AppServices` assembly — and declares every
...
`DKNet.Accounts.Infra/Services/EventPublisher.cs` drains the queue and calls `IMessageBus.Publish(...)` after
...
`DKNet.Accounts.Api/Configs/FluentValidationConfig.cs`, and enforced by SharpGrip's
...
`DKNet.Accounts.Infra` references the `SlimMessageBus.Host.FluentValidation` package, but nothing in
...
`DKNet.Accounts.Infra/Features/AutomatedSample/ExternalEvents/ProductCreatedNotificationHandler.cs`:
...
2. Write `THandler` as a `Fluents.EventsConsumers.IHandler<TEvent>` under
   `DKNet.Accounts.Infra/Features/<Feature>/ExternalEvents/`. External-system concerns belong in `Infra`, not
...
| Condition | Where |
|---|---|
| `FeatureManagement:EnableServiceBus` is `true` | `DKNet.Accounts.Share/Options/FeatureOptions.cs` |
| `ConnectionStrings:AzureBus` is non-empty | `DKNet.Accounts.Infra/Extensions/ServiceBusSetup.cs` |
