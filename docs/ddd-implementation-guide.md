...
How to add one vertical-slice feature to a solution generated by `dotnet new dknet-dknet.accounts`,
end to end — domain entity, EF Core mapping, domain event, application action, endpoint,
and its tests.

> **The `Product` and `PurchaseOrder` samples below are fictional.** They come from the
> `DKNet.Templates` solution template this service was scaffolded from; **no such entity, slice or
> `/v1/products` route exists in this repository**. Read them as illustrations of the two shapes,
> never as code or calls you will find here. This service's own slices are `AccountGroups`,
> `Accounts`, `Postings` and `Currencies` under `ApiEndpoints/DKNet.Accounts.Domains/Features/` and
> `ApiEndpoints/DKNet.Accounts.AppServices/`, and which of its routes take the automated shape is
> recorded in [the README](../README.md#which-routes-are-generated-and-which-are-hand-written).

Two samples ground every step below, built two different ways:

- **`ManualSample`/`PurchaseOrder`** (template sample, not in this repo) — every layer hand-written. This guide's primary
...
- **`AutomatedSample`/`Product`** (template sample, not in this repo) — entity/events/CRUD declared via attributes, everything the
  generators can produce is produced. Called out inline wherever it diverges from the manual
  walkthrough.

`docs/samples/manual-vs-automated.md` in `DKNet.Templates` states,
layer by layer, what the automated shape costs you before you pick it.

> Path of truth: `src/ApiEndpoints/` (inner projects are prefixed `DKNet.Accounts.*` in this repo;
...
|---|-------|-----------------|
| 1 | Layer map | — |
| 2 | Domain entity | `DKNet.Accounts.Domains/Features/<Feature>/Entities/` |
| 3 | EF Core mapping | `DKNet.Accounts.Infra/Features/<Feature>/Mappers/` |
| 4 | Application action | `DKNet.Accounts.AppServices/<Feature>/V1/Actions/` |
| 5 | Query specs | `DKNet.Accounts.AppServices/<Feature>/V1/Specs/` |
| 6 | DTO and Mapster wiring | `DKNet.Accounts.AppServices/<Feature>/V1/<Feature>Dto.cs` |
| 7 | Lazy mapping | `DKNet.SlimBus.Extensions.LazyMapper` |
| 8 | Domain event and handler | `DKNet.Accounts.AppServices/<Feature>/V1/Events/` |
| 9 | Endpoint | `DKNet.Accounts.Api/ApiEndpoints/<Feature>V1Endpoint.cs` |
| 10 | Unit / integration tests | `DKNet.Accounts.App.Tests/Integration/<Feature>/V1/` |
| 11 | BDD tests | `DKNet.Accounts.App.BDDTests/Features/<Domain>/` |
...
![Architecture diagram of a scaffolded solution: DKNet.Accounts.AppHost orchestrates and DKNet.Accounts.Api dispatches through IMessageBus into DKNet.Accounts.AppServices, which calls DKNet.Accounts.Domains aggregates; DKNet.Accounts.Infra supplies IRepositorySpec and the event publisher to AppServices and the EF Core mapping and seeding to Domains; DKNet.Accounts.Share is read by every layer, and the DKNet.Accounts.App.Tests/Architecture project holds NetArchTest shape rules over the Api, AppServices, Infra and Domains projects.](diagrams/templates-solution-layers.svg)

```
DKNet.Accounts.Api          entry point, endpoints, auth, OpenAPI
  ↓
DKNet.Accounts.AppServices   CQRS actions: request + validator + handler, DTOs, event handlers
  ↓
DKNet.Accounts.Domains       entities, aggregate roots
  ↑
DKNet.Accounts.Infra         EF Core mapping, seed data, repositories, event publisher, message bus
DKNet.Accounts.Share         constants/options read by every layer
DKNet.Accounts.AppHost       Aspire orchestration only (Redis + PostgreSQL + Api project)
```
...
Reference: `DKNet.Accounts.Domains/Features/ManualSample/Entities/PurchaseOrder.cs`
...
**Automated alternative** (`DKNet.Accounts.Domains/Features/AutomatedSample/Entities/Product.cs`): the
...
Reference: `DKNet.Accounts.Infra/Features/ManualSample/Mappers/PurchaseOrderConfigs.cs`
...
- No manual `DbSet` and no manual `modelBuilder.ApplyConfiguration(...)` call anywhere. Every
  ...
  `UseAutoConfigModel`, wired in **both** `DKNet.Accounts.Infra/Extensions/InfraSetup.cs`
...
- Optional seed data: drop an `IDataSeedingConfiguration<T>` next to the mapper, under
  `StaticData/` (see `DKNet.Accounts.Infra/Features/ManualSample/StaticData/PurchaseOrderStaticData.cs`).
...
- After adding or changing a mapper, generate the migration from `src/ApiEndpoints/`:
  `./add-migration.sh <Name>` (wraps `dotnet ef migrations add -c CoreDbContext -p DKNet.Accounts.Infra/DKNet.Accounts.Infra.csproj`).
...
Reference: `DKNet.Accounts.AppServices/ManualSample/V1/Actions/Create.cs`
...
handler for you — see `DKNet.Accounts.AppServices/obj/Generated/DKNet.SlimBus.Generators/.../ProductCrudRequests.g.cs`
...
Reference: `DKNet.Accounts.AppServices/ManualSample/V1/Specs/SpecGetPurchaseOrder.cs`
...
`filter`/`search`/`orderBy`/`desc`/`pageNumber`/`pageSize` query surface resolved against the DTO, plus
`fromDate`/`toDate` bounds that limit a bare call over audited records to the last three months of
activity — the full contract is [`generic-list-endpoint.md`](generic-list-endpoint.md). What it cannot
do is express a *bespoke* predicate the way a hand-written spec can.

## 6. DTO and Mapster wiring — `DKNet.Accounts.AppServices/<Feature>/V1/<Feature>Dto.cs`

Reference (hand-written): `DKNet.Accounts.AppServices/ManualSample/V1/PurchaseOrderDto.cs`
...
**Automated alternative** (`DKNet.Accounts.AppServices/AutomatedSample/V1/ProductDto.cs`):
...
Reference (hand-raised): `DKNet.Accounts.Domains/Features/ManualSample/Entities/PurchaseOrderCreatedEvent.cs`
+ `DKNet.Accounts.AppServices/ManualSample/V1/Events/PurchaseOrderCreatedEventHandler.cs`
...
- The event is a plain `sealed record`. `entity.AddEvent(...)` in the constructor/method queues it;
  `DKNet.Accounts.Infra/Services/EventPublisher.cs` forwards every queued event onto
...
- A subscriber is any class implementing `Fluents.EventsConsumers.IHandler<TEvent>`. Put it in
  ...
  (`DKNet.Accounts.Infra/Features/<Feature>/ExternalEvents/`, e.g. `ProductCreatedNotificationHandler.cs`)
  for anything backed by an external system. No manual DI registration is needed — both projects'
  assemblies are scanned by `AddServiceBus` in `DKNet.Accounts.Infra/Extensions/ServiceBusSetup.cs`.
...
compiled assembly (`strings bin/**/DKNet.Accounts.Domains.dll | grep <Entity>`) before wiring a consumer.
...
Reference (hand-mapped): `DKNet.Accounts.Api/ApiEndpoints/ManualSample/PurchaseOrderV1Endpoint.cs`
...
- `IEndpointConfig` comes from the `DKNet.AspCore.Extensions` NuGet package, not this repo. Every
  route here is a **literal** call against the raw dknet.accounts-API surface (`group.MapPost(...)`, etc.).
...
**Automated alternative** (`DKNet.Accounts.Api/ApiEndpoints/AutomatedSample/ProductV1Endpoint.cs`):
...
> **Validation gap to know before you pick this shape.** .NET 10's automatic dknet.accounts-API validation
...
> enforced without checking which mapping style its endpoint uses.
>
> **That confirmation was made against the `DKNet.Templates` `Product` sample, not against this
> service** — `/v1/products` does not exist here. The mechanism still applies: this service's generated
> routes (`PUT /v1/account-groups/{id}` and its siblings — see
> [the README](../README.md#which-routes-are-generated-and-which-are-hand-written)) are registered
> through the same generic wrappers, so do not assume a `DataAnnotations` attribute on a generated
> request property is evaluated. The refusals this service guarantees are the `422` codes in the
> README's error table, and every one of them is enforced in a hand-written handler.
> Full detail in
> `DKNet.Templates`' `docs/samples/manual-vs-automated.md`.

## 10. Unit / integration tests — `DKNet.Accounts.App.Tests/Integration/<Feature>/V1/`

- `ApiFixture` (`DKNet.Accounts.App.Tests/Integration/Support/ApiFixture.cs`) boots the real host once per
...
- Domain events land off the request thread (the in-memory bus runs with blocking publish
  ...
  (`DKNet.Accounts.App.TestSupport/Eventually.cs`) instead of asserting immediately after `Send`.
- Shape rules — internal/sealed classes, no public entity property in a DTO, max length on every
  ...
  separately by `DKNet.Accounts.App.Tests/Architecture/*` (NetArchTest). A new feature that violates one of
...
- Coverage target is at least 80% on every class you touch. `src/coverage.runsettings` scopes
  collection to `[DKNet*]`/`[DKNet.Accounts*]` and excludes `*Tests`/`bin`/`obj`/`GlobalUsings.cs` — don't
...
- The BDD host runs with `RequireAuthorization = false`, so `[FromClaim]` properties fall back to
  ...
  comes from `DKNet.Accounts.Api/appsettings.Testing.json` — the base `appsettings.json` ships
...
dotnet test src/ApiEndpoints/DKNet.Accounts.App.Tests/DKNet.Accounts.App.Tests.csproj --filter "FullyQualifiedName~<Feature>"
dotnet test src/ApiEndpoints/DKNet.Accounts.App.BDDTests/DKNet.Accounts.App.BDDTests.csproj --filter "TestCategory=<Feature>"
```
