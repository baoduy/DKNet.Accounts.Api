...
```csharp
// DKNet.Accounts.Infra/Extensions/ServiceBusSetup.cs
service.AddSlimBusEfCoreInterceptor<CoreDbContext>()
    .AddSlimMessageBus(mbb => { ... });
```

Both samples publish through the same interceptor and the same
`DKNet.Accounts.Infra/Services/EventPublisher.cs`. The only difference between them is *how* the event
gets raised.

## Manual style — raise it yourself

`PurchaseOrder` (`DKNet.Accounts.Domains/Features/ManualSample/Entities/PurchaseOrder.cs`) calls `AddEvent`
...
`DKNet.Accounts.Domains/Features/ManualSample/Entities/PurchaseOrderCreatedEvent.cs`:
...
`DKNet.Accounts.AppServices/ManualSample/V1/Events/PurchaseOrderCreatedEventHandler.cs`, an
...
`Product` (`DKNet.Accounts.Domains/Features/AutomatedSample/Entities/Product.cs`) never calls `AddEvent`.
...
```bash
strings src/ApiEndpoints/DKNet.Accounts.Domains/bin/Debug/net10.0/DKNet.Accounts.Domains.dll | grep Event
```
...
`DKNet.Accounts.AppServices/AutomatedSample/V1/Events/ProductEventHandlers.cs`'s
