...
`DKNet.Accounts.AppServices/ManualSample/V1/Specs/SpecGetPurchaseOrder.cs` builds one predicate that serves
...
`DKNet.Accounts.AppServices/Share/Generics/ModelSpecGenericStatusCounts.cs` shows a spec parameterized over
...
shapes work. Two worked examples live in `DKNet.Accounts.AppServices/ManualSample/V1/Queries/`:
...
choices**, not the package's, and this query keeps these values whatever the package does. The
generated route has an entirely separate contract: it *clamps* an out-of-range page size rather than
rejecting it, its own default and ceiling are `1000`, and it adds `fromDate`/`toDate` activity bounds
— see [Generic List Endpoint](generic-list-endpoint.md).
...
`DKNet.Accounts.Api/Configs/Endpoints/StatusCountsEndpointMapperExtensions.cs` provides
...
No current `DKNet.Accounts.Api.ApiEndpoints` config actually calls `MapGetStatusCounts`, so there is no live
...
`DKNet.Accounts.App.Tests/Integration/StatusCounts/StatusCountsEndpointMapperExtensionsTests.cs`, which
