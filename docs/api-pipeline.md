...
`Strict-Transport-Security` is **not** emitted here — `DKNet.Accounts.Api/Configs/HttpsConfig.cs` owns that
...
`DKNet.Accounts.Api/Configs/CrosConfig.cs` reads the `Cors:AllowedOrigins` string array and is
...
- `DKNet.Accounts.Api/Configs/AppConfig.cs` only calls `AddAuthConfig()` (JWT bearer auth,
  `DKNet.Accounts.Api/Configs/Auth/AuthConfig.cs`) when it's `true`.
...
remarks on `DKNet.Accounts.App.Tests/Integration/Support/AuthOnApiFixture.cs`.
...
pieces of code in `DKNet.Accounts.Api`.
...
implements it — `DKNet.Accounts.Api/Configs/Handlers/HttpContextSensitiveDataPrincipalAccessor.cs`:
...
`DKNet.Accounts.Api/Configs/RateLimits/RateLimitConfig.cs`: a chained `PartitionedRateLimiter` combining a
...
The partition key comes from `DKNet.Accounts.Api/Configs/RateLimits/RateLimitKeyProvider.cs`:
...
- **Health checks** (`FeatureManagement:EnableHealthCheck`, default `true`) — a `CoreDbContext`
  ...
  `DKNet.Accounts.Api/Configs/Healthz/HealthzConfig.cs` at three paths:
...
- **OpenAPI/Scalar** (`FeatureManagement:EnableSwagger`, default `false`) —
  `DKNet.Accounts.Api/Configs/Swagger/SwaggerConfig.cs` maps the OpenAPI 3.0 document and a Scalar UI at
