...
import { CurrenciesScreen } from '@/components/admin/CurrenciesScreen';
...
/**
 * `GET /currencies` — DRK-1697 §3 row 13, a thin server wrapper: session + config check,
 ...
 */
export default async function CurrenciesPage(): Promise<JSX.Element> {
  ...
  return (
    <AppShell
      ...
      breadcrumb={<Breadcrumb items={[{ label: 'Administration' }, { label: 'Currencies' }]} />}
      topbarRight={
        <>
    ...
    >
      <CurrenciesScreen grantedScopes={grantedScopes} />
    </AppShell>
  );
}
