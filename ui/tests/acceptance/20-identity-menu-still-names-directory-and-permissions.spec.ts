 ...
 *
 * The menu content carrying no `style` attribute is the "new base controls" RED signal
 * (see 19-frame-survives-the-rebuild.spec.ts): today `UserMenu`'s `role="menu"` panel is a
 * large inline `style` object; a shadcn `DropdownMenuContent` is styled by class name.
 */
...
test('The identity menu still names the directory and the permissions', async ({ page, baseURL }) => {
  expect(MISSING_SCOPES).toEqual(['postings.reverse']);
